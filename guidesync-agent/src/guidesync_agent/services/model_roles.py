...
def model_role_settings_from_env(
    ...
    *,
    ...
    model = os.environ.get(f"{prefix}_MODEL") or fallback_model
    base_url = os.environ.get(f"{prefix}_BASE_URL")
    if base_url is None and role is not ModelRole.ORCHESTRATOR:
    ...
    return ModelRoleSettings(
        ...
        provider_family=parse_provider_family(
            os.environ.get(f"{prefix}_PROVIDER_FAMILY")
        ...
        timeout_seconds=int(os.environ.get(f"{prefix}_TIMEOUT_SECONDS") or fallback_timeout),
        thinking=parse_optional_thinking(
            os.environ.get(f"{prefix}_THINKING"),
            fallback_thinking,
        ),
...
def provider_config_for_role(
    ...
    *,
    ...
    role_settings = model_role_settings_from_env(role, fallback=fallback)
    metadata = {
        **(fallback.metadata if fallback else {}),
    ...
    return ProviderConfig(
        ...
        name=role_settings.name or (fallback.name if fallback else None),
        base_url=role_settings.base_url,
        api_key_env=role_settings.api_key_env,
        api_key=api_key_from_env(role_settings.api_key_env, fallback),
...
def attach_role_metadata(config: ProviderConfig, role: ModelRole) -> ProviderConfig:
    role_settings = model_role_settings_from_env(role, fallback=config)
    role_metadata = {
        **role_settings.evidence_metadata(),
