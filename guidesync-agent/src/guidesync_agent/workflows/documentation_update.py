...
def prepare_documentation_update_workflow(
    ...
    for repository in request.repositories:
        ...
        if result.ok:
            changed_files.extend(item.model_dump(mode="json") for item in result.files)
        elif result.error:
...
def attach_retrieved_docs_to_update(
    ...
    for result in retrieved_docs[:5]:
        path = result.node.path or result.chunk.path if result.chunk else result.node.path
